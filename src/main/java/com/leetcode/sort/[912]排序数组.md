...
+ 时间复杂度：![O(N\logN) ](./p__O_N_log_N__.png) ，这里 *N* 是数组的长度；
...
+ 时间复杂度：![O(N\logN) ](./p__O_N_log_N__.png) ，这里 *N* 是数组的长度；
+ 空间复杂度：![O(\logN) ](./p__O_log_N__.png) ，这里占用的空间主要来自递归函数的栈空间。
...
+ 堆排序是选择排序的优化，选择排序需要在未排定的部分里通过「打擂台」的方式选出最大的元素（复杂度 *O(N)*），而「堆排序」就把未排定的部分构建成一个「堆」，这样就能以 ![O(\logN) ](./p__O_log_N__.png)  的方式选出最大元素；
